...
//        $encPwd = hash('sha256', $usrPwd);
        ...
        $aiishId = strip_tags($POST["aiishId"]);
        $stateId = strip_tags($POST["stateId"]);
        $districtId = strip_tags($POST["district"]);
...
//        $status = TRUE;
        ...
        $st = strip_tags($GET["st"]);
        $stName = ($st == 1)?"Active":"Deactive";
        $user_id = decode($GET["add-user"]);
        
        $qry_1 = "UPDATE `user_tb` SET `user_status` = '$st' WHERE `user_id` = '$user_id' ";
        mysql_query("INSERT INTO `activity_log_tb` (`ip_address`, `user`, `activity_date_time`, `activity_page`, `page_activity`) VALUES('{$_SERVER["REMOTE_ADDR"]}', '".USERAUTH."', '".date("d-m-Y h:i:sa")."', 'add-user', 'Status changed of Id-$user_id to $stName')");
        mysql_query($qry_1);
        $status = TRUE;
...
function getUserList(){
    ...
    $qry_user = "SELECT `user_id`, `user_name`, `role_id`, `user_email`, `user_password`, `aiish_id`, `state_id`, `district_id`, `user_status`, `user_created_on`, `user_updated_on`, `user_created_by`, `user_updated_by` FROM `user_tb`";
    ...
    while($rw_user = mysql_fetch_array($result_user)){
        ...
        $data .= "
            <tr>
                ...
                <td>".  getAIISHCenterNameByID($rw_user["aiish_id"])."</td>
                <!--<td>". getStateNameByStateId($rw_user["state_id"])."</td>
                <td>". getDistNameByDistId($rw_user["district_id"])."</td> -->
...
function getimpression($patient, $abshrr){
    ...
    if($patientDet["rt_screen1"] == 2  && $patientDet["lt_screen1"] == 2){
            $screen1Refer = "Refer";
        }
        elseif ($patientDet["rt_screen1"] == 2  && empty ($patientDet["lt_screen1"] )) {
            $screen1Refer = "Refer";
        }
        elseif ( empty ($patientDet["rt_screen1"] )  && $patientDet["lt_screen1"] == 2) {
            $screen1Refer = "Refer";
        }
        
    if($patientDet["rt_screen1"] == 2  && $patientDet["lt_screen1"] == 2){
            $screenoneRefer = "Refer";
        }
        elseif ($patientDet["rt_screen1"] == 2  && empty ($patientDet["lt_screen1"] )) {
            $screenoneRefer = "Refer";
        }
        ...
        elseif ($patientDet["rt_screen1"] == 1  && $patientDet["lt_screen1"] == 2) {
            $screenoneRefer = "Refer";
        }
        elseif ($patientDet["rt_screen1"] == 2  && $patientDet["lt_screen1"] == 1) {
            $screenoneRefer = "Refer";
        }
    ...
    if ($patientDet["hrr_type"] == 2 && $screen1Pass == "pass" && $screen2Pass == "pass" && $passBOA == "pass" && $acanalPass == "pass" && $primRefPass == "pass") {
         $s1 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='1' ";
        $q1 = mysql_query($s1);
        ...
        echo "<div class='row'>
                <div class='col-md-12'>
                    <div class='col-md-7 impSel' id=''>
                        <h3><span class='label label-success' >$msg1</span></h3>
                    </div>
                    <div class='col-md-5'>
                        ".SelectImpresion($patient, $r1['imp_id'])."
                    </div>
                </div>
              </div>  
        ";
    ...
    elseif ($patientDetail["hrr_type"] == 2 && !empty($screen1Pass) && !empty($passBOA) && !empty($acanalPass) && !empty($primRefPass)) {
         $s1 = "select `imp_name`, `imp_id` from `tbl_impression` where `imp_id`='1' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
        $msg1 = $r1['imp_name'];
    ...
    elseif ($patientDet["hrr_type"] == 1 && !empty($screen1Pass) && !empty($screen2Pass) && !empty($passBOA) && !empty($acanalPass) && !empty($primRefPass)) {
        $s1 = "select `imp_name`, `imp_id` from `tbl_impression` where `imp_id`='3' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
        $msg1 = $r1['imp_name'];
        ...
        $hrrType  = ($patientDet["hrr_type"] == 1)?"<p><h4>Present</h4></p>":"";
...
//        $otherList = (empty($nbn500PassOne) && empty($nbn500PassTwo) && $nbn4000PassOne && $nbn4000PassTwo && $whiteNoisyPassOne && $whiteNoisyPassTwo)?"-":"";
    ...
    elseif ($patientDet["hrr_type"] == 1 && !empty($screen1Pass) && empty ($patientDet["rt_screen2"]) && empty ($patientDet["lt_screen2"])  && !empty($passBOA) && !empty($acanalPass) && !empty($primRefPass)) {
        $s1 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='3' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
        $msg1 = $r1['imp_name'];
...
//        $otherList = (empty($nbn500PassOne) && empty($nbn500PassTwo) && $nbn4000PassOne && $nbn4000PassTwo && $whiteNoisyPassOne && $whiteNoisyPassTwo)?"-":"";
    ...
    elseif ($patientDet["hrr_type"] == 1 && !empty($screen2Pass) && !empty($referBOA) && !empty($acanalPass) && !empty($primRefPass)) {
        if(!empty($screen1Refer)){
        $s1 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='3' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
        $msg1 = $r1['imp_name'];
...
//        $otherList = (empty($nbn500PassOne) && empty($nbn500PassTwo) && $nbn4000PassOne && $nbn4000PassTwo && $whiteNoisyPassOne && $whiteNoisyPassTwo)?"-":"";
    ...
    elseif ($patientDet["hrr_type"] == 2 && $patientDet["rt_screen1"] == 2 && $patientDet["lt_screen1"] == 2 && !empty ($screen2Pass) && !empty($passBOA) && !empty($acanalPass) && !empty($primRefPass)) {
        if(empty($aabrPass) || !empty($aabrPass)){
        $s1 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='3' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
...
//        $otherList = (empty($nbn500PassOne) && empty($nbn500PassTwo) && $nbn4000PassOne && $nbn4000PassTwo && $whiteNoisyPassOne && $whiteNoisyPassTwo)?"-":"";
    ...
    elseif ($patientDet["hrr_type"] == 2 && $patientDet["rt_screen1"] == 2 && $patientDet["lt_screen1"] == 2 && $patientDet["rt_screen2"] == 3 && $patientDet["lt_screen2"] == 3 && !empty($referBOA) && !empty($acAbnormal) && !empty($primRefPass)) {
        if(!empty($aabrPass)){
        $s1 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='3' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
...
//        $otherList = (empty($nbn500PassOne) && empty($nbn500PassTwo) && $nbn4000PassOne && $nbn4000PassTwo && $whiteNoisyPassOne && $whiteNoisyPassTwo)?"-":"";
        ...
        $acanal_normal = !empty($patientDet["normal_val"])?"<li>Normal </li>":"Abnormal";
    ...
    elseif ($patientDet["hrr_type"] == 2 && $patientDet["rt_screen1"] == 2 && $patientDet["lt_screen1"] == 2 && $patientDet["rt_screen2"] == 2 && $patientDet["lt_screen2"] == 2 && !empty($referBOA) && !empty($acanalPass) && !empty($primRefPass)) {
        if(!empty($aabrPass)){
        $s1 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='3' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
...
//        $otherList = (empty($nbn500PassOne) && empty($nbn500PassTwo) && $nbn4000PassOne && $nbn4000PassTwo && $whiteNoisyPassOne && $whiteNoisyPassTwo)?"-":"";
   ...
   // elseif ($patientDet["hrr_type"] == 2 && $patientDet["rt_screen1"] == 1 && $patientDet["lt_screen1"] == 2 && $patientDet["rt_screen2"] == 3 && $patientDet["lt_screen2"] == 3 && !empty($passBOA) && !empty($acanalPass) && !empty($primRefPass)) {
    elseif ($patientDet["hrr_type"] == 2 && !empty ($screenoneRefer) && $patientDet["rt_screen2"] == 3 && $patientDet["lt_screen2"] == 3 && !empty($passBOA) && !empty($acanalPass) && !empty($primRefPass)) {
        if(!empty($aabrPass)){
        $s1 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='3' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
...
//        $otherList = (empty($nbn500PassOne) && empty($nbn500PassTwo) && $nbn4000PassOne && $nbn4000PassTwo && $whiteNoisyPassOne && $whiteNoisyPassTwo)?"-":"";
    ...
    elseif ($patientDet["hrr_type"] == 2 && $patientDet["rt_screen1"] == 2 && $patientDet["lt_screen1"] == 2 && $patientDet["rt_screen2"] == 1 && $patientDet["lt_screen2"] == 2 && !empty($passBOA) && !empty($acanalPass) && !empty($primRefPass)) {
        if(!empty($aabrPass)){
        $s1 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='3' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
...
//        $otherList = (empty($nbn500PassOne) && empty($nbn500PassTwo) && $nbn4000PassOne && $nbn4000PassTwo && $whiteNoisyPassOne && $whiteNoisyPassTwo)?"-":"";
    ...
    elseif ($patientDet["hrr_type"] == 2 && $patientDet["rt_screen1"] == 1 && $patientDet["lt_screen1"] == 1 && empty($patientDet["rt_screen2"]) && empty($patientDet["lt_screen2"]) && !empty($referBOA) && !empty($acanalPass) && !empty($primRefPass)) {
        if(!empty($aabrPass)){
        $s1 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='3' ";
        $q1 = mysql_query($s1);
        $r1 = mysql_fetch_assoc($q1);
...
//        $otherList = (empty($nbn500PassOne) && empty($nbn500PassTwo) && $nbn4000PassOne && $nbn4000PassTwo && $whiteNoisyPassOne && $whiteNoisyPassTwo)?"-":"";
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <p><h5>1st Screening</h5></p>
                                <p>$oaeright_pass$oaeright_Refer$oaeright_CNT</p>
                                <p>$oaeleft_pass$oaeleft_Refer$oaeleft_CNT</p>
    ...
    . "<div class='row'>
        ...
        $s12 = "select `imp_id`, `imp_name` from `tbl_impression` where `imp_id`='2' ";
        $q12 = mysql_query($s12);
    ...
    echo "
        <div class='row'>
            <div class='col-md-12 form-group'>
                ...
                <div class='col-md-5'>
                    ".SelectImpresion($patient, $r12["imp_id"])."
                </div>
            </div>
...
function SubmitRole($POST, $GET){
    ...
    if(isset($POST["SubmitRole"])){
        $roleType = strip_tags($POST["roleName"]);
        $roleId = strip_tags($POST["role_id"]);
        
        if(empty($roleId)){
            $qry_1 = "INSERT INTO `user_role_tb` (`role_type`, `role_created_on`, `role_created_by`) VALUES('$roleType', NOW(), '".USERAUTH."')";
        }
        else{
            $qry_1 = "UPDATE `user_role_tb` SET `role_type` = '$roleType', `role_updated_on` = NOW(), `role_updated_by` = '".USERAUTH."' WHERE `role_id` = '$roleId' ";
        }
        //mysql_query($qry_1);
        
        $status = TRUE;
    }else if(isset($GET["create-role"]) && is_numeric(decode($GET["create-role"])) && isset($GET["st"]) && is_numeric($GET["st"])){
        $st = strip_tags($GET["st"]);
        $stName = ($st == 1)?"Active":"Deactive";
        $role_id = decode($GET["create-role"]);
        
        $qry_1 = "UPDATE `user_role_tb` SET `role_status` = '$st' WHERE `role_id` = '$role_id' ";
        mysql_query("INSERT INTO `activity_log_tb` (`ip_address`, `user`, `activity_date_time`, `activity_page`, `page_activity`) VALUES('{$_SERVER["REMOTE_ADDR"]}', '".USERAUTH."', '".date("d-m-Y h:i:sa")."', 'create-role', 'Status changed of Id-$role_id to $stName')");
        mysql_query($qry_1);
        $status = TRUE;
...
//        $status = TRUE;
//    }
    if(isset($GET["add-state"]) && is_numeric(decode($GET["add-state"])) && isset($GET["st"]) && is_numeric($GET["st"])){
        $st = strip_tags($GET["st"]);
        $stName = ($st == 1)?"Active":"Deactive";
        $state_id = decode($GET["add-state"]);
        
        $qry_1 = "UPDATE `tbl_states` SET `state_status` = '$st' WHERE `state_id` = '$state_id' ";
        mysql_query("INSERT INTO `activity_log_tb` (`ip_address`, `user`, `activity_date_time`, `activity_page`, `page_activity`) VALUES('{$_SERVER["REMOTE_ADDR"]}', '".USERAUTH."', '".date("d-m-Y h:i:sa")."', 'add-state', 'Status changed of state_id - $state_id to $stName')");
        $status = TRUE;
...
function getStateList(){
    ...
    while($rw_state = mysql_fetch_array($result_state)){
        ...
        $stateUpdateBy = !empty($rw_state["state_updated_by"])?"".getAuthNameByID(encode($rw_state["state_updated_by"]))."(". getRoleNameByAuthID(encode($rw_state["state_updated_by"])).")":"";
        $data .= "
            <tr>
...
//        $status = TRUE;
//    }
    if(isset($GET["add-district"]) && is_numeric(decode($GET["add-district"])) && isset($GET["st"]) && is_numeric($GET["st"])){
        $st = strip_tags($GET["st"]);
        $stName = ($st == 1)?"Active":"Deactive";
        $district_id = decode($GET["add-district"]);
        
        $qry_1 = "UPDATE `tbl_districts` SET `dist_status` = '$st' WHERE `dist_id` = '$district_id' ";
        mysql_query("INSERT INTO `activity_log_tb` (`ip_address`, `user`, `activity_date_time`, `activity_page`, `page_activity`) VALUES('{$_SERVER["REMOTE_ADDR"]}', '".USERAUTH."', '".date("d-m-Y h:i:sa")."', 'add-district', 'Status changed of Id-$district_id to $stName')");
        $status = TRUE;
...
function SubmitTaluk($POST, $GET){
    ...
    if(isset($GET["add-taluk"]) && is_numeric(decode($GET["add-taluk"])) && isset($GET["st"]) && is_numeric($GET["st"])){
        $st = strip_tags($GET["st"]);
        $stName = ($st == 1)?"Active":"Deactive";
        $taluk_id = decode($GET["add-taluk"]);
        
        $qry_1 = "UPDATE `tbl_cities` SET `city_status` = '$st' WHERE `city_id` = '$taluk_id' ";
        mysql_query("INSERT INTO `activity_log_tb` (`ip_address`, `user`, `activity_date_time`, `activity_page`, `page_activity`) VALUES('{$_SERVER["REMOTE_ADDR"]}', '".USERAUTH."', '".date("d-m-Y h:i:sa")."', 'add-taluk', 'Status changed of Id -$taluk_id to $stName')");
        $status = TRUE;
    ...
    while ($file = readdir($directry)) { 
        ...
        if (eregi("\.php",$file)) { # Look at only files with a .php extension
          $data .= "<tr>
                        <td>$index</td>
                        <td>$pageName</td>
                        <td><div class='col-md-6'><select class='form-control' id='rolesel$index' onchange = 'roleSelect(\"$index\", \"$pageName\", \"\")' name='roles'><option disabled selected value>Select Role</option>".getRoleSelectList()."</select></div></td>
                        <td>
                            <div id='DvPageID$index'>
                                <label class='control-label radio-inline'><input type='radio' size='' name='gender$index' class='roleRad$index' id='male' value='male' disabled=''  >View</label>
                                <label class='control-label radio-inline'><input type='radio' size='' name='gender$index' class='roleRad$index ' id='male' value='male' disabled=''  >Edit</label>
                                <label class='control-label radio-inline'><input type='radio' size='' name='gender$index' class='roleRad$index ' id='male' value='male' disabled=''  >Restricted</label>
                            </div>
                        </td>    
                    </tr>
...
//AIISH CENTER
function SubmitAIISHCenter($POST, $GET){
    $formData = "";
    $status = FALSE;
    
    
    if(isset($GET["add-aiish"]) && is_numeric(decode($GET["add-aiish"])) && isset($GET["st"]) && is_numeric($GET["st"])){
        $st = strip_tags($GET["st"]);
        $stName = ($st == 1)?"Active":"Deactive";
        $aiishID = decode($GET["add-aiish"]);
        
        $qry_1 = "UPDATE `tbl_aiish` SET `aiish_status` = '$st' WHERE `aiish_id` = '$aiishID' ";
        $message = "Status updated successfully";
        mysql_query("INSERT INTO `activity_log_tb` (`ip_address`, `user`, `activity_date_time`, `activity_page`, `page_activity`) VALUES('{$_SERVER["REMOTE_ADDR"]}', '".USERAUTH."', '".date("d-m-Y h:i:sa")."', 'add-aiish', 'Status changed of Id-$aiishID to $stName')");
        $status = TRUE;
    }else if( isset($GET["add-aiish"]) && is_numeric(decode($GET["add-aiish"])) && !isset($GET["st"]) ){
        $id =  decode($GET["add-aiish"]);
        $formData = getAIISHListsByID($id);
    ...
    if($status){
        ...
        redirect(HostRoot."add-aiish");
    }
    
    return $formData;
}

function getAIISHCenterList(){
    $data = "";
    
    $qry_aiish = "SELECT `aiish_id`, `aiish_name`, `aiish_state_id`, `aiish_district_id`, `aiish_status`, `aiish_created_by`, `aiish_created_on`, `aiish_updated_by`, `aiish_updated_on` FROM `tbl_aiish`";
    $result_aiish = mysql_query($qry_aiish);
    
    while($rw_aiish = mysql_fetch_array($result_aiish)){
        $ID = encode($rw_aiish["aiish_id"]);
        $st = ($rw_aiish['aiish_status'] == 1) ? array("st"=>"2", "name"=>"Inactive") : array("st"=>"1", "name"=>"Active");        
        $aiishCreatedBy = !empty($rw_aiish["aiish_created_by"])?"".getAuthNameByID(encode($rw_aiish["aiish_created_by"]))."(". getRoleNameByAuthID(encode($rw_aiish["aiish_created_by"])).")":"";
        $aiishUpdateBy = !empty($rw_aiish["aiish_updated_by"])?"".getAuthNameByID(encode($rw_aiish["aiish_updated_by"]))."(". getRoleNameByAuthID(encode($rw_aiish["aiish_updated_by"])).")":"";
        $data .= "
            <tr>
                <td>{$rw_aiish["aiish_name"]}</td>
                <td>".getStateNameByStateId($rw_aiish["aiish_state_id"])."</td>
                <td>".getDistNameByDistId($rw_aiish["aiish_district_id"])."</td>
                <td>{$aiishCreatedBy}</td>
                <td>{$rw_aiish["aiish_created_on"]}</td>
                <td>{$aiishUpdateBy}</td>
                <td>{$rw_aiish["aiish_updated_on"]}</td>
                <td>".getStatusInfo($rw_aiish["aiish_status"])."</td>
                <td><a href='".HostRoot."add-aiish/{$ID}'><i class='fa fa-gears'></i> Edit</a> &nbsp;|&nbsp;
                    <a href='".HostRoot."add-aiish/{$ID}/st/{$st["st"]}'><i class='fa fa-trash-o'></i> {$st["name"]}</a></td>
            </tr>
...
function SubmitNBSCenter($POST, $GET){
    $formData = "";
    $status = FALSE;
    
    if(isset($GET["add-nbs"]) && is_numeric(decode($GET["add-nbs"])) && isset($GET["st"]) && is_numeric($GET["st"])){
        $st = strip_tags($GET["st"]);
        $stName = ($st == 1)?"Active":"Deactive";
        $nbsID = decode($GET["add-nbs"]);
        
        $qry_1 = "UPDATE `tbl_nbs` SET `nbs_status` = '$st' WHERE `nbs_id` = '$nbsID' ";
        mysql_query("INSERT INTO `activity_log_tb` (`ip_address`, `user`, `activity_date_time`, `activity_page`, `page_activity`) VALUES('{$_SERVER["REMOTE_ADDR"]}', '".USERAUTH."', '".date("d-m-Y h:i:sa")."', 'add-nbs', 'Status changed of nbs_id - $nbsID to $stName')");
        $message = "Status updated successfully";
        $status = TRUE;
    }else if( isset($GET["add-nbs"]) && is_numeric(decode($GET["add-nbs"])) && !isset($GET["st"]) ){
        $id =  decode($GET["add-nbs"]);
        $formData = getNBSListsByID($id);
    ...
    if($status){
        ...
        redirect(HostRoot."add-nbs");
    }
    
    return $formData;
}

function getNBSHospitalList(){
    $data = "";
    
    $qry_nbs = "SELECT `nbs_id`, `nbs_name`, `aiish_id`, `nbs_state_id`, `nbs_district_id`, `nbs_status`, `nbs_created_by`, `nbs_created_on`, `nbs_updated_by`, `nbs_updated_on` FROM `tbl_nbs`";
    $result_nbs = mysql_query($qry_nbs);
    
    while($rw_nbs = mysql_fetch_array($result_nbs)){
        $ID = encode($rw_nbs["nbs_id"]);
        $st = ($rw_nbs['nbs_status'] == 1) ? array("st"=>"2", "name"=>"Inactive") : array("st"=>"1", "name"=>"Active");        
        $nbsCreatedBy = !empty($rw_nbs["nbs_created_by"])?"".getAuthNameByID(encode($rw_nbs["nbs_created_by"]))."(". getRoleNameByAuthID(encode($rw_nbs["nbs_created_by"])).")":"";
        $nbsUpdateBy = !empty($rw_nbs["nbs_updated_by"])?"".getAuthNameByID(encode($rw_nbs["nbs_updated_by"]))."(". getRoleNameByAuthID(encode($rw_nbs["nbs_updated_by"])).")":"";
        $data .= "
            <tr>
                <td>{$rw_nbs["nbs_name"]}</td>
                <td>".getAIISHCenterNameByID($rw_nbs["aiish_id"])."</td>
                <td>".getStateNameByStateId($rw_nbs["nbs_state_id"])."</td>
                <td>".getDistNameByDistId($rw_nbs["nbs_district_id"])."</td>
                <td>{$nbsCreatedBy}</td>
                <td>{$rw_nbs["nbs_updated_on"]}</td>
                <td>{$rw_nbs["nbs_created_on"]}</td>
                <td>{$nbsUpdateBy}</td>
                <td>".getStatusInfo($rw_nbs["nbs_status"])."</td>
                <td><a href='".HostRoot."add-nbs/{$ID}'><i class='fa fa-gears'></i> Edit</a> &nbsp;|&nbsp;
                    <a href='".HostRoot."add-nbs/{$ID}/st/{$st["st"]}'><i class='fa fa-trash-o'></i> {$st["name"]}</a></td>
            </tr>
...
//OS Center
function SubmitOSCenter($POST, $GET){
    $formData = "";
    $status = FALSE;
    
    if(isset($GET["add-osc"]) && is_numeric(decode($GET["add-osc"])) && isset($GET["st"]) && is_numeric($GET["st"])){
        $st = strip_tags($GET["st"]);
        $stName = ($st == 1)?"Active":"Deactive";
        $oscID = decode($GET["add-osc"]);
        
        $qry_1 = "UPDATE `tbl_osc_centers` SET `osc_status` = '$st' WHERE `osc_id` = '$oscID' ";
        mysql_query("INSERT INTO `activity_log_tb` (`ip_address`, `user`, `activity_date_time`, `activity_page`, `page_activity`) VALUES('{$_SERVER["REMOTE_ADDR"]}', '".USERAUTH."', '".date("d-m-Y h:i:sa")."', 'add-osc', 'Status changed of osc_id - $oscID to $stName')");
        $message = "Status updated successfully";
        $status = TRUE;
    }else if( isset($GET["add-osc"]) && is_numeric(decode($GET["add-osc"])) && !isset($GET["st"]) ){
        $id =  decode($GET["add-osc"]);
        $formData = getOSCListsByID($id);
    ...
    if($status){
        ...
        redirect(HostRoot."add-osc");
    }
    
    return $formData;
}

function getOSCenterList(){
    $data = "";
    
    $qry_osc = "SELECT `osc_id`, `osc_names`, `aiish_id`, `osc_state_id`, `osc_district_id`, `osc_status`, `osc_created_by`, `osc_created_on`, `osc_updated_by`, `osc_updated_on` FROM `tbl_osc_centers`";
    $result_osc = mysql_query($qry_osc);
    
    while($rw_osc = mysql_fetch_array($result_osc)){
        $ID = encode($rw_osc["osc_id"]);
        $st = ($rw_osc['osc_status'] == 1) ? array("st"=>"2", "name"=>"Inactive") : array("st"=>"1", "name"=>"Active");        
        $oscCreatedBy = !empty($rw_osc["osc_created_by"])?"".getAuthNameByID(encode($rw_osc["osc_created_by"]))."(". getRoleNameByAuthID(encode($rw_osc["osc_created_by"])).")":"";
        $oscUpdateBy = !empty($rw_osc["osc_updated_by"])?"".getAuthNameByID(encode($rw_osc["osc_updated_by"]))."(". getRoleNameByAuthID(encode($rw_osc["osc_updated_by"])).")":"";
        $data .= "
            <tr>
                <td>{$rw_osc["osc_names"]}</td>
                <td>".getAIISHCenterNameByID($rw_osc["aiish_id"])."</td>
                <td>".getStateNameByStateId($rw_osc["osc_state_id"])."</td>
                <td>".getDistNameByDistId($rw_osc["osc_district_id"])."</td>
                <td>{$oscCreatedBy}</td>
                <td>{$rw_osc["osc_created_on"]}</td>
                <td>{$rw_osc["osc_updated_on"]}</td>
                <td>{$oscUpdateBy}</td>
                <td>".getStatusInfo($rw_osc["osc_status"])."</td>
                <td><a href='".HostRoot."add-osc/{$ID}'><i class='fa fa-gears'></i> Edit</a> &nbsp;|&nbsp;
                    <a href='".HostRoot."add-osc/{$ID}/st/{$st["st"]}'><i class='fa fa-trash-o'></i> {$st["name"]}</a></td>
            </tr>
...
//Hospital Center
function SubmitHospital($POST, $GET){
    $formData = "";
    $status = FALSE;
    
    if(isset($POST["submitHospitalInfo"])){
        $hospName = strip_tags($POST["hospitalName"]);
        $hospAbbr = strip_tags($POST["hospAbb"]);
        $stateId = strip_tags($POST["stateId"]);
        $distId = strip_tags($POST["district"]);
        $hospID = strip_tags($POST["hospID"]);
        
//        if(empty($hospID)){
//            $qry_1 = "INSERT INTO `tbl_hospital` (`hosp_name`, `hosp_abbr`, `hosp_state_id`, `hosp_dist_id`, `hosp_created_on`, `hosp_created_by`) VALUES('$hospName', '$hospAbbr', '$stateId', '$distId', NOW(), '".USERAUTH."')";
//        
//            $message = "New Hospital added successfully.";
//        }
//        else{
//            $qry_1 = "UPDATE `tbl_hospital` SET `hosp_name` = '$hospName', `hosp_abbr` = '$hospAbbr', `hosp_state_id` = '$stateId', `hosp_dist_id` = '$distId', `hosp_updated_on` = NOW(), `hosp_updated_by` = '".USERAUTH."' WHERE `hosp_id` = '$hospID' ";
//        
...
//        }
    ...
    else if(isset($GET["add-hospital"]) && is_numeric(decode($GET["add-hospital"])) && isset($GET["st"]) && is_numeric($GET["st"])){
        $st = strip_tags($GET["st"]);
        $hospID = decode($GET["add-hospital"]);
        $stName = ($st == 1)?"Active":"Deactive";
        
        $qry_1 = "UPDATE `tbl_hospital` SET `hosp_status` = '$st' WHERE `hosp_id` = '$hospID' ";
        $message = "Status updated successfully";
        
        mysql_query("INSERT INTO `activity_log_tb` (`ip_address`, `user`, `activity_date_time`, `activity_page`, `page_activity`) VALUES('{$_SERVER["REMOTE_ADDR"]}', '".USERAUTH."', '".date("d-m-Y h:i:sa")."', 'add-hospital', 'Status changed of hosp_id - $hospID to $stName')");
        
        $status = TRUE;
    }else if( isset($GET["add-hospital"]) && is_numeric(decode($GET["add-hospital"])) && !isset($GET["st"]) ){
        $id =  decode($GET["add-hospital"]);
        $formData = getHospitalByID($id);
    ...
    if($status){
        ...
        redirect(HostRoot."add-hospital");
    }
...
function getHospitalList(){
    ...
    $qry_hosp = "SELECT `hosp_id`, `hosp_name`, `hosp_abbr`, `aiish_id`, `hosp_state_id`, `hosp_dist_id`, `hosp_status`, `hosp_created_by`, `hosp_created_on`, `hosp_updated_by`, `hosp_updated_on` FROM `tbl_hospital`";
    $result_hosp = mysql_query($qry_hosp);
    
    while($rw_hosp = mysql_fetch_array($result_hosp)){
        $ID = encode($rw_hosp["hosp_id"]);
        $st = ($rw_hosp['hosp_status'] == 1) ? array("st"=>"2", "name"=>"Inactive") : array("st"=>"1", "name"=>"Active");        
        $hospCreatedBy = !empty($rw_hosp["hosp_created_by"])?"".getAuthNameByID(encode($rw_hosp["hosp_created_by"]))."(". getRoleNameByAuthID(encode($rw_hosp["hosp_created_by"])).")":"";
        $hospUpdateBy = !empty($rw_hosp["hosp_updated_by"])?"".getAuthNameByID(encode($rw_hosp["hosp_updated_by"]))."(". getRoleNameByAuthID(encode($rw_hosp["hosp_updated_by"])).")":"";
        $data .= "
            <tr>
                <td>{$rw_hosp["hosp_name"]}</td>
                <td>{$rw_hosp["hosp_abbr"]}</td>
                <td>".getAIISHCenterNameByID($rw_hosp["aiish_id"])."</td>
                <td>".getStateNameByStateId($rw_hosp["hosp_state_id"])."</td>
                <td>".getDistNameByDistId($rw_hosp["hosp_dist_id"])."</td>
                <td>{$hospCreatedBy}</td>
                <td>{$rw_hosp["hosp_created_on"]}</td>
                <td>{$rw_hosp["hosp_updated_on"]}</td>
                <td>{$hospUpdateBy}</td>
                <td>".getStatusInfo($rw_hosp["hosp_status"])."</td>
                <td><a href='".HostRoot."add-hospital/{$ID}'><i class='fa fa-gears'></i> Edit</a> &nbsp;|&nbsp;
                    <a href='".HostRoot."add-hospital/{$ID}/st/{$st["st"]}'><i class='fa fa-trash-o'></i> {$st["name"]}</a></td>
            </tr>
